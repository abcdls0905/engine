...
bool Game_Scene::Load(const char* pstrfile,IScene* pScene,IWorld* pWorld)
{
	m_pWorld = pWorld;
...
{	 
	...
	//先创建一个场景
	IIniFile* pIni = GetCore()->CreateIniFile(pstrfile);
	...
	ITerrain* pTerrain = (ITerrain*) GetCore()->GetEntity(terrain_id);
...
void Game_Scene::load_material(const char* pstrfile,ITerrain* pTerrain,IScene* pScene)
{
	...
	IEntity* pEntity = (IEntity*) GetCore()->GetEntity(material_id);
...
void Game_Scene::load_particle_system(const char* pstrfile,ITerrain* pTerrain,IScene* pScene)
{
	...
	IEntity* pParticleMan = (IEntity*)GetCore()->GetEntity(particle_man_id);
...
void Game_Scene::load_weather(const char* pstrfile,ITerrain* pTerrain,IScene* pScene)
{
	...
	IWeather* pWeather = (IWeather*) GetCore()->GetEntity(weather_id);
...
void Game_Scene::load_skybox(IIniFile* pIni,ITerrain* pTerrain,IScene* pScene)
{
	...
	ISkyBox* pSkyBox = (ISkyBox*) GetCore()->GetEntity(skybox_id);
...
void Game_Scene::load_sun(IIniFile* pIni,ITerrain* pTerrain,IScene* pScene)
{
	...
	ISunGlow* pSunGlow = (ISunGlow*) GetCore()->GetEntity(sunglow_id);
...
void Game_Scene::load_shadow_system(ITerrain* pTerrain,IScene* pScene)
{
	...
	m_pShadowManager =  (IShadowManager*) GetCore()->GetEntity(shadow_man);
...
{ 
	...
	RoleComposite* pRale = (RoleComposite*)GetCore()->CreateEntity("RoleComposite");
...
void Game_Scene::load_screen_config(const char* pstrfile,ITerrain* pTerrain,IScene* pScene)
{
	...
	IIniFile* pIni = GetCore()->CreateIniFile(mbzTemp);
	pIni->SetFileName(mbzTemp);
	bool b = pIni->LoadFromFile();
	if(!b)
	{
...
void Game_Scene::load_ppfilter(const char* pstrfile,ITerrain* pTerrain,IScene* pScene)
{
	...
	IEntity* pEntity = (IEntity*) GetCore()->GetEntity(post_effect_filter_id);
	if(pEntity == NULL)
		return;
...
void Game_Scene::load_dynamic_weather_system(const char* pstrfile,ITerrain* pTerrain,IScene* pScene)
{
	...
	IVisBase* pVisBase = (IVisBase*) GetCore()->GetEntity(dynamic_weather_id);
...
{
	IEntity* pEntity = (IEntity*) GetCore()->GetEntity(water_id);
...
{
	IEntity* pEntity = (IEntity*) GetCore()->GetEntity(grass_id);
...
{
	ISeaWater* pSeaWater = (ISeaWater*) GetCore()->GetEntity(water_id);
