...
{
  IVarTable* pCustom = pEntity->GetCustoms();
	IVar* pVar = pEntity->GetCustoms()->GetValue(name);
...
IEntity* CHelper::GetEntity(const PERSISTID& id)
{
...
IVisBase* CHelper::GetPlayer()
{
  SceneObjManager* sceneMgr = (SceneObjManager*)GetGlobalEntity(GLOBAL_SCENEMANAGER);
  if (sceneMgr)
  {
    PERSISTID& id = sceneMgr->GetPlayerId();
    return (IVisBase*)g_pCore->GetEntity(id);
  }
  return NULL;
}
...
IActor* CHelper::GetActor(IVisBase* pRole)
{
  if (!IsActor(pRole))
  {    
    Assert(false);
...
bool CHelper::IsActor(IVisBase* pRole)
{
  if (!pRole)
    return false;
  return pRole->GetEntInfo()->IsKindOf("Actor");
}


static StateMachine*	pStateMachine = NULL;

StateMachine* CHelper::GetStateMachine()
{
  if (pStateMachine == NULL)
  {
    pStateMachine = (StateMachine*)GetGlobalEntity(GLOBAL_PROP_ENT_StateMachine);
  }
