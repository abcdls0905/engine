...
CWalkFlagCompress::CWalkFlagCompress()
{
	m_pRoot = NULL;
	m_nScale = 0;
	m_pResultData = NULL;
...
void CWalkFlagCompress::ReleaseAll()
{
	if (m_pRoot)
	{
		DeleteNode(m_pRoot);
...
void CWalkFlagCompress::DeleteNode(node_data_t* node)
{
	Assert(node != NULL);
...
bool CWalkFlagCompress::Compress(size_t scale, const unsigned char* pdata, 
	size_t size)
{
...
CWalkFlagCompress::node_data_t* CWalkFlagCompress::CreateNode(size_t scale, 
...
size_t CWalkFlagCompress::CompressNode(node_data_t* node, 
	const unsigned char* pdata, size_t size)
{
	...
	if (node->nType == NODE_COMPRESS)
	{
		...
		unsigned int value = pdata[index] & 0xF;

		m_pResultData[m_nResultSize] = (value << 4) + (unsigned int)(node->nType);
		m_nResultSize++;
		count++;
	}
...
bool CWalkFlagCompress::Decompress(size_t scale, const unsigned char* pdata, 
	size_t size)
{
	Assert(pdata != NULL);
...
bool CWalkFlagCompress::DecompressData(size_t scale, size_t row, size_t col,
	const unsigned char* pdata, size_t size, size_t& pos)
{
	...
	else if (type == NODE_ORIGINAL)
	{
		...
		for (size_t r = 0; r < scale; ++r)
		{
			for (size_t c = 0; c < scale; ++c)
			{
				size_t pos_byte = (r * scale + c) >> 1;
				size_t index = (row + r) * m_nScale + col + c;

				if ((index & 0x1) == 0)
				{
					m_pResultData[index] = pdata[pos + pos_byte] & 0xF;
				}
				else
				{
					m_pResultData[index] = (pdata[pos + pos_byte] >> 4) & 0xF;
				}
			}
		}
...
bool CWalkFlagCompress::GetFlag(size_t scale, size_t row, size_t col,
...
bool CWalkFlagCompress::GetFlagData(size_t scale, size_t row, size_t col, 
	const unsigned char* pdata, size_t size, size_t& pos, unsigned char& value)
{
	...
	else if (type == NODE_ORIGINAL)
	{
		...
		size_t pos_byte = (row * scale + col) >> 1;

		if ((col & 0x1) == 0)
		{
			value = pdata[pos + pos_byte] & 0xF;
		}
		else
		{
			value = (pdata[pos + pos_byte] >> 4) & 0xF;
		}
	}
	else if (type == NODE_BRANCH)
	{
