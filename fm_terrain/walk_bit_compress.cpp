...
CWalkBitCompress::CWalkBitCompress()
{
	m_pRoot = NULL;
	m_nScale = 0;
	m_pResultData = NULL;
...
void CWalkBitCompress::ReleaseAll()
{
	if (m_pRoot)
	{
		DeleteNode(m_pRoot);
...
void CWalkBitCompress::DeleteNode(node_data_t* node)
{
	Assert(node != NULL);
...
bool CWalkBitCompress::Compress(size_t scale, const unsigned char* pdata, 
	size_t size)
{
...
CWalkBitCompress::node_data_t* CWalkBitCompress::CreateNode(size_t scale, 
...
size_t CWalkBitCompress::CompressNode(node_data_t* node, 
...
bool CWalkBitCompress::Decompress(size_t scale, const unsigned char* pdata, 
	size_t size)
{
	Assert(pdata != NULL);
...
bool CWalkBitCompress::DecompressData(size_t scale, size_t row, size_t col,
	const unsigned char* pdata, size_t size, size_t& pos)
{
	...
	else if (type == NODE_ORIGINAL)
	{
		...
		size_t origin_size = scale * scale / 8;
		...
		for (size_t r = 0; r < scale; ++r)
		{
			for (size_t c = 0; c < scale; ++c)
			{
				size_t k = r * scale + c;
				size_t pos_byte = k >> 3;
				size_t pos_bit = k & 0x7;
				unsigned char value = 
					(unsigned char)((pdata[pos + pos_byte] >> pos_bit) & 0x1);
				size_t index = (row + r) * m_nScale + col + c;
...
bool CWalkBitCompress::GetBit(size_t scale, size_t row, size_t col,
							  ...
							  unsigned char& value)
{
	...
	return GetBitData(scale, row, col, pdata, size, pos, value);
}

bool CWalkBitCompress::GetBitData(size_t scale, size_t row, size_t col, 
								  const unsigned char* pdata, size_t size, 
								  size_t& pos, unsigned char& value)
{
	Assert(row < scale);
	Assert(col < scale);
	...
	else if (type == NODE_ORIGINAL)
	{
		...
		size_t pos_byte = index >> 3;
		size_t pos_bit = index & 0x7;

		if ((pdata[pos + pos_byte] & (0x1 << pos_bit)) != 0)
		{
			value = 1;
		}
		else
	...
	else if (type == NODE_BRANCH)
	{
		...
		if (row < hscale)
		{
			if (col < hscale)
			{
				return GetBitData(hscale, row, col, pdata, size, pos, 
					value);
			}
			else
			{
				...
				return GetBitData(hscale, row, col - hscale, pdata, size, 
					pos, value);
			}
		}
		else
		{
			if (col < hscale)
			{
				...
				return GetBitData(hscale, row - hscale, col, pdata, size, 
					pos, value);
			}
			else
			{
				...
				return GetBitData(hscale, row - hscale, col - hscale, 
