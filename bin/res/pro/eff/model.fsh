...
#ifdef SHADOWMAP
varying mediump vec4 oViewPos;
uniform mediump float c_ShadowParam;
uniform mediump vec4 c_farDistance;
uniform mediump int c_ShadowMapCount;
varying mediump vec4 v_shadowPos1;
#ifdef PSSM
varying mediump vec4 v_shadowPos2;
varying mediump vec4 v_shadowPos3;
varying mediump vec4 v_shadowPos4;
#endif
#endif
...
void main (void)
{
...
#ifdef KEEPOUT
	gl_FragColor = texture2D(tex_KeepoutMap, oTex0.xy, bias);
#else
#ifdef ONLYZPASS
	#ifdef CLIPPLANE
	...
	#else
		#ifdef LIGHTMAP
			diffuse_color.xyz *= texture2D(tex_LightMap, oTex0.zw, bias).xyz * c_vLightMapColorScale.xyz;
		#endif
	#endif
...
//		mediump vec4 shadow_Inten = texture2D(tex_Shadow, tex_uv, bias);
            ...
            {
                shadow_depth = texture2D(tex_Shadow1, v_shadowPos1.xy).x;

                // if(shadow_depth > 0.0 && shadow_depth < 1.0)
                //  shadow_depth = c_ShadowParam;
            }
            #ifdef PSSM
            else
            {
                ...
                {
                    shadow_depth = texture2D(tex_Shadow1, v_shadowPos1.xy).x;
                    shadow_depth = shadow_depth < v_shadowPos1.z ? c_ShadowParam : 1.0;
                }
                else if(oViewPos.z < c_farDistance.y)
                {
                    shadow_depth = texture2D(tex_Shadow2, v_shadowPos2.xy).x;
                    shadow_depth = shadow_depth < v_shadowPos2.z ? c_ShadowParam : 1.0;
                }
                else if(oViewPos.z < c_farDistance.z)
                {
                    shadow_depth = texture2D(tex_Shadow3, v_shadowPos3.xy).x;
                    shadow_depth = shadow_depth < v_shadowPos3.z ? c_ShadowParam : 1.0;
                }
                else if(oViewPos.z < c_farDistance.w)
                {
                    shadow_depth = texture2D(tex_Shadow4, v_shadowPos4.xy).x;
                    shadow_depth = shadow_depth < v_shadowPos4.z ? c_ShadowParam : 1.0;
                }
            }
            #endif
            mediump vec4 shadow_Inten = vec4(shadow_depth);
		#endif
	#endif
	...
	mediump vec3 light_dir = c_vLightDir;
		...
		mediump vec4 crTexDiffuse = texture2D(tex_Diffuse, oTex0.xy, bias);
			...
			crTexDiffuse.a = texture2D(tex_DiffuseAlpha, oTex0.xy, bias).r;
		#endif
