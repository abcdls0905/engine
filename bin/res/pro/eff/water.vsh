...
varying highp vec3 oSunToVertex;
...
#if defined(VETEX_WAVE)   
   mediump float wavedistance = distance(posWorld.xyz, c_ViewPosition.xyz);
 
   Wave Waves[5];
   
   Waves[0].fFreq = c_WaveParam0.x;
   Waves[1].fFreq = c_WaveParam1.x;
   Waves[2].fFreq = c_WaveParam2.x;
   Waves[3].fFreq = c_WaveParam3.x;
   Waves[4].fFreq = c_WaveParam4.x;
   Waves[0].fAmp = c_WaveParam0.y;
   Waves[1].fAmp = c_WaveParam1.y;
   Waves[2].fAmp = c_WaveParam2.y;
   Waves[3].fAmp = c_WaveParam3.y;
   Waves[4].fAmp = c_WaveParam4.y;
   Waves[0].fPhase = c_WaveParam0.z;
   Waves[1].fPhase = c_WaveParam1.z;
   Waves[2].fPhase = c_WaveParam2.z;
   Waves[3].fPhase = c_WaveParam3.z;
   Waves[4].fPhase = c_WaveParam4.z;
   Waves[0].fK = c_WaveParam0.w;
   Waves[1].fK = c_WaveParam1.w;
   Waves[2].fK = c_WaveParam2.w;
   Waves[3].fK = c_WaveParam3.w;
   Waves[4].fK = c_WaveParam4.w;
   Waves[0].vDir = vec2(c_WaveParam5.xy);
   Waves[1].vDir = vec2(c_WaveParam5.zw);
   Waves[2].vDir = vec2(c_WaveParam6.xy);
   Waves[3].vDir = vec2(c_WaveParam6.zw);
   Waves[4].vDir = vec2(c_WaveParam7.xy);

   for(int i = 0 ; i < 5; i++ )
   {
	  mediump float baseVal = max(abs(sin(dot(Waves[i].vDir, posWorld.xz) * Waves[i].fFreq + c_fTime * Waves[i].fPhase) * 0.5 + 0.500001), 0.0001);

      #if !defined(WAVELESS)
        // pos.y += EvaluateWaveSharp(Waves[i], posWorld.xz, c_fTime);
         pos.y = pos.y + Waves[i].fAmp * pow(baseVal, Waves[i].fK);
      #endif

	  mediump float diff = Waves[i].fK * Waves[i].fFreq * Waves[i].fAmp 
		        * pow(baseVal, Waves[i].fK - 1.0) 
