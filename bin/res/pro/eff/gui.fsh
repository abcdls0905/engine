...
void main()
{
	...
	#ifdef USE_PICTURE 
		#ifdef USE_OUTLINE
			highp float fontAlpha = texture2D(tex_BaseMap, oTexCoord).y;
			highp float outlineAlpha = texture2D(tex_BaseMap, oTexCoord).x;
			
			if ((fontAlpha + outlineAlpha) > 0.0){
				vec4 color = oDiffuse * fontAlpha + vec4(c_outline_color, 1.0) * (1.0 - fontAlpha);
				output_color = vec4( color.rgb,max(fontAlpha,outlineAlpha)*color.a);
			}
			else {
				discard;
			}
