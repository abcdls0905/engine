...
// #endif
	...
	float inten = saturate(1.0 - c_Attenu.y * d) / max(c_Attenu.z * d * d, 1.0);
#else
	float inten = max(0.0, 1.0 - c_Attenu.y * d);
#endif	
...
// #endif
	...
	vec3 diffuse;
	vec3 specular;
	calc_light(diffuse, specular, tex_uv, -normalize(oViewScale.xyz), to_light, inten * c_LightColor.rgb, shadow);
