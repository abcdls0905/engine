...
varying mediump vec4 oTexCoord45;
	...
	#if defined(LIGHTMAP) && defined(WARFOG)
		varying mediump vec4 oTexCoordLB;
	#else
...
void main()
{
   ...
   crTex += crDtail3 * crBlend.y;
	...
	shadowInten = final_shadow_color(shadowInten.x);
...
#if defined(FOGEXP)
	mediump float fogexp = 1.0 / exp(oDepth.x * c_FogExpParam.w);
 	oColor.xyz = c_FogExpParam.xyz*(1.0 - fogexp) + oColor.xyz * fogexp;  
#endif
...
#if defined(WARFOG)
	#if defined(LIGHTMAP)
		mediump vec2 vTexCoord = oTexCoordLB.zw;
	#else
		mediump vec2 vTexCoord = oTexCoordLB.xy;
	#endif
	
	mediump vec4 crWarFog = texture2D(tex_WarFog, vTexCoord);
	
	oColor.xyz = oColor.xyz * mix(0.2,1.0,crWarFog.w);
#endif
