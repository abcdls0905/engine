...
CPixelShader::CPixelShader(Render* pRender)
{
	Assert(pRender != NULL);
...
bool CPixelShader::Create(const void* shader_data, unsigned int shader_size)
{
...
#endif
	glslopt_shader* shader = glslopt_optimize (ctx,kGlslOptShaderFragment,shader_code,0);
	//检查是否进行优化成功
