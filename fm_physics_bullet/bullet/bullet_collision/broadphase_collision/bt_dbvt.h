...
#include "linear_math/bt_aligned_object_array.h"
...
在bullet的远距阶段是很高效的碰撞检测数据结构（比较OOB，K- DOP）。是组成dbvtbroadphase的重要成员。
*/
...
/* btDbvtNode				*/ 
/*
很明显这是一个典型的二叉树节点，同时当node是内部节点时将指向2个子结点，node是叶子节点时，将指向用户定义数据.
*/
struct	btDbvtNode
{
	btDbvtVolume	volume;
...
struct	btDbvt
{
	...
	int				m_lkhd;
	int				m_leaves;
	unsigned		m_opath;    // 将在优化中使用，主要用于纪录通往特定节点的路径。
...
//负责进行两个dbvt子树的比较，找出两个子树中重叠的节点对，但是基于的是一个局部栈（函数调用结束则释放）。
DBVT_PREFIX
...
/*
 的算法主要是遍历两个目标子树所有可能重合的节点对（基于栈的遍历）只要两个节点中有一个不是叶子
 节点则压入栈中直到两个节点都为叶子节点且相互重叠，都调用预先制定好的碰撞逻辑来处理。
*/
DBVT_PREFIX
...
//
/*
这个函数的主要算法是遍历所有和目标节点重合的叶子节点，然后使用已有的处理逻辑模块 
btDbvtTreeCollider  来处理这两个节点。会引入新的数据结构btHashedOverlappingPairCache,
可以参见后 续的btDbvtBroadphase分析
负责在一个树中搜索和对应包围体重叠的节点。
*/
DBVT_PREFIX
								  ...
								  DBVT_IPOLICY) const
{
	DBVT_CHECKTYPE
		if(root)
		{
			...
			do	{
				const btDbvtNode*	n=stack[stack.size()-1];// 取栈顶元素  
				stack.pop_back();                           // 栈顶元素出栈  
				if(Intersect(n->volume,volume))             // 如果两个AABB有相交  
				{
